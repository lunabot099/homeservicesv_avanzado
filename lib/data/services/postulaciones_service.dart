...
class PostulacionesService {
  ...
  Future<PostulacionSolicitudModel> create(PostulacionSolicitudModel p) async {
    final data = await _client.from(_table).insert(p.toMap()).select().single();
  ...
  Future<List<PostulacionSolicitudModel>> getBySolicitud(
    ...
    return (data as List)
        .map(
            (e) => PostulacionSolicitudModel.fromMap(e as Map<String, dynamic>))
        .toList();
  }
  ...
  Future<void> updateEstado(
      String postulacionId, EstadoPostulacion estado) async {
    await _client
        .from(_table)
        .update({'estado': estado.name}).eq('id', postulacionId);
  }
  ...
  Stream<List<PostulacionSolicitudModel>> streamBySolicitud(
    ...
    return _client
        ...
        .eq('solicitud_id', solicitudId)
        .map((rows) =>
            rows.map((e) => PostulacionSolicitudModel.fromMap(e)).toList());
  }
